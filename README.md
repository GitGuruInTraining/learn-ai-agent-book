...
**Agent = LLM + 上下文 + 工具**——本书围绕这个核心公式，用 10 章把 AI Agent 从原理讲到工程实战。全书正文、配图、配套实验开源，欢迎亲手把实验跑一遍。

> 📢 **2.0 版变更（相较 1.4 版）**：原第四章中的“异步交互”与原第九章中的“多模态 Agent”合并为新的第六章“交互：观察与动作空间的扩展”。原第六、七、八章依次后移为第七、八、九章。

| 📚 **10 章** 正文，从基础到生产 | 📂 **103 个** 配套实验（含本地项目与外部复现轨道） |
| :---: | :---: |

中文正文在仓库根目录：[`introduction.md`](introduction.md)、[`chapter1.md`](chapter1.md) ~ [`chapter10.md`](chapter10.md)、[`afterword.md`](afterword.md)、[`reference-answers.md`](reference-answers.md)。配图在 [`images/`](images/)。
...
| 章 | 主题 | 一句话核心 | 正文 | 实验 |
| :--: | --- | --- | :--: | :--: |
| 1 | 🚀 **AI Agent 入门** | **Agent = LLM + 上下文 + 工具**；Harness 工程才是竞争力 | [读](chapter1.md) | [4](chapter1/README.md) |
| 2 | 🎯 **上下文工程** | 上下文决定能力上限：KV Cache、提示工程、Agent Skills、上下文压缩 | [读](chapter2.md) | [10](chapter2/README.md) |
| 3 | 📚 **用户记忆和知识库** | 跨会话记住用户、接入外部知识：用户记忆、RAG、结构化索引、知识图谱 | [读](chapter3.md) | [12](chapter3/README.md) |
| 4 | 🛠️ **工具** | 工具是 Agent 的双手：MCP 协议、感知/执行/协作三类工具与主动工具发现 | [读](chapter4.md) | [5](chapter4/README.md) |
| 5 | 💻 **Coding Agent 与通用 Agent** | 代码是「能创造新工具的工具」，生产级 Coding Agent 全景 | [读](chapter5.md) | [16](chapter5/README.md) |
| 6 | 🎙️ **交互：观察与动作空间的扩展** | 从模态与时序两个维度扩展 Agent 的观察与动作空间：异步与事件驱动、语音交互、Computer Use 和机器人操作 | [读](chapter6.md) | [13](chapter6/README.md) |
| 7 | 🎯 **Agent 的评估** | 把表现变成可比较信号：评估环境、指标、统计显著性、评估驱动选型 | [读](chapter7.md) | [14](chapter7/README.md) |
| 8 | 🧠 **模型后训练** | 预训练/SFT/RL 三阶段：何时选 SFT、何时选 RL，工具调用内化、样本效率 | [读](chapter8.md) | [19](chapter8/README.md) |
| 9 | 🔄 **Agent 的持续进化** | 从运行轨迹获得学习信号，更新知识、指令、程序与参数 | [读](chapter9.md) | [9](chapter9/README.md) |
| 10 | 🤝 **多 Agent 协作** | 群体智能高于个体：协作框架、上下文共享/隔离、涌现的「Agent 社会」 | [读](chapter10.md) | [6](chapter10/README.md) |
...
| 类型 | 说明 |
| --- | --- |
| 📝 **书籍内容改进** | 勘误、补充、更清晰的表述，或新增前沿进展（正文见 `chapter*.md`） |
| 🐛 **代码改进与 Bug 修复** | 让配套项目更健壮、更易用、更贴近生产实践 |
| 🧪 **新的实践项目** | 为某个实验补充/替换更好的实现，或贡献全新的示例项目 |
| 🎨 **配图设计改进** | 直接改进 `images/` 中已签入的 SVG 图表，让它们更清晰美观 |
