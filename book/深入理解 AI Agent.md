...
> [!tip] 阅读提示
> 本章是全书的概念地图——它会快速引入 Agent 的核心公式、运行循环、工程框架和设计模式，为后续章节提供统一的术语和参照坐标。初次阅读时不必逐一记住所有概念，建议先建立整体印象；后续每一章都会展开讲解本章提到的某一个方面，届时可随时回来对照。
...
> [!example] 实验 1-2 ★：Kimi K3 原生 Agent 能力
>
...
> [!example] 实验 1-3 ★：GPT-5.6 原生 Deep Research 能力
>
...
> [!example] 实验 1-4 ★：文生图工作流与原生图像生成的对照
>
...
> [!example] 实验 2-1 ★：本地 LLM 服务部署与工具调用
>
>
> ![[images/fig2-5.svg]]
> *图2-5 本地 LLM 工具调用架构*
...
> [!warning] 技术门槛提示
> 本节涉及 Transformer 注意力机制和 KV Cache 的内部原理，是全书技术密度最高的部分之一。如果你不熟悉这些底层机制，**可以跳过原理细节，只需记住以下三条核心结论**：
>
...
### 3.1 从 API 消息到模型 Token：Chat Template
...
### 3.3 KV Cache 与 Prompt Cache：两个层级的缓存
...
### 3.5 KV Cache 未必是一次性的：可编辑、可组合的“笔记”
...
> [!example] 实验 2-6 ★★：使用 Agent Skills 从论文生成演示文稿
>
...
> [!example] 实验 2-7 ★★：从个人范文创建“去 AI 味”写作 Skill
>
...
## 6 Agent 状态栏：通过元信息增强 Agent 轨迹管理
...
> [!example] 实验 2-8 ★★：通过注意力可视化验证 Agent 状态栏的效果
>
...
> [!example] 实验 2-9 ★★：几种好用的 Agent 状态栏技术
>
...
> [!example] 实验 3-4 ★★：构建向量检索服务：ANN 索引算法的比较研究
>
...
> [!example] 实验 3-5 ★★：探究稀疏检索：从零实现 BM25 搜索引擎
>
...
> [!example] 实验 3-6 ★★：混合检索流水线：结合稀疏、稠密与重排序
>
...
> [!example] 实验 3-7 ★★★：结构化索引：RAPTOR 与 GraphRAG 的知识组织哲学
>
...
> [!example] 实验 3-8 ★★：智能体化 RAG 与非智能体化 RAG 的对比研究
>
...
> [!example] 实验 3-9 ★★：利用智能体化 RAG 构建用户记忆
>
...
> [!example] 实验 3-10 ★★：上下文感知检索：解决 RAG 的上下文丢失问题
>
...
> [!example] 实验 3-11 ★★★：利用上下文感知检索增强用户记忆
>
...
> [!example] 实验 3-12 ★★★：从结构化数据中提取隐性知识：以司法判例分析为例
>
...
![[images/fig4-4.svg]]
*图4-4 动态发现后的上下文结构：工具 schema 散落在轨迹各处*
...
> [!example] 实验 4-3 ★★：多模态信息提取：三种技术范式的对比分析
>
...
### 1.4 Harness 工程在 Coding Agent 中的实践
...
> [!example] 实验 5-3 ★★：使用代码生成工具提升数学解题能力
>
...
> [!example] 实验 5-4 ★★：使用代码生成工具提升逻辑思考能力
>
...
> [!example] 实验 5-5 ★★：小模型通过代码化知识提升执行规则的准确性
>
...
> [!example] 实验 5-9 ★★：同一个零件的两种生成路线——代码与生成模型
>
...
> [!example] 实验 5-13 ★★：自然语言交互的 ERP Agent
>
...
> [!example] 实验 5-15 ★★★：动态生成软件的权限内嵌数据对象
>
...
> [!example] 实验 5-16 ★★★：开发一个能创造 Agent 的 Agent
>
...
![[images/fig6-1.svg]]
*图6-1 事件驱动的异步 Agent 架构*

### 2.2 OpenClaw 的事件驱动机制实现
...
> [!example] 实验 6-1 ★★★：事件驱动的邮件处理 Agent
>
>
> ![[images/fig6-3.svg]]
> *图6-3 实验 6-1 事件驱动 Agent 架构*
>
>
...
> [!example] 实验 6-2 ★★★：带并行执行和打断能力的异步 Agent
>
>
> ![[images/fig6-5.svg]]
> *图6-5 实验 6-2 异步 Agent 打断与恢复*
>
>
...
> [!example] 实验 6-5 ★★：本地运行 MiniCPM-o 4.5，对比端到端与自级联
>
...
![[images/fig6-11.svg]]
*图6-11 Computer Use Agent 的感知-思考-行动循环*
...
> [!example] 实验 6-7 ★：运行 Computer Use（Anthropic 参考路径或开放模型路径）
>
> 路径 A 使用 Anthropic Computer Use Demo：容器打包完整的 Ubuntu 桌面环境（含浏览器、终端等常用工具），前端接收任务，后端把指令与截图发送给 Claude，再执行模型返回的鼠标、键盘、终端或编辑动作。
>
> 路径 B 使用本书的 [`../code/chapter6/computer-use-open-model`](../code/chapter6/computer-use-open-model/) 示例代码：默认以开放权重的 Qwen3-VL 32B Instruct 驱动 browser-use，可通过 OpenRouter 托管 API，也可连接自托管的 vLLM/SGLang 等服务。
...
![[images/fig6-13.svg]]
*图6-13 Set-of-Mark 与结构化元素索引（browser-use 实现）*
...
> [!example] 实验 6-8 ★：使用 browser-use 实现自动浏览器操作
>
...
### 4.3 能看动画、能听声音的 Computer Use Agent
...
## 5 机器人操作：以 XLeRobot 整理桌面为例

> [!tip] 阅读提示
> 本节始终使用同一个任务——“把红色杯子放进托盘，把黄色废纸放进垃圾盒，最后重新观察并确认桌面状态”。实验 6-9 和 6-11 是 XLeRobot 真机实验，需要机械臂、标定、急停装置和现场观察员；实验 6-10、6-12 和 6-13 是对应的本地 GPU 实验。真机与模拟会明确分开报告，但任务目标、动作语义和成功条件保持一致。
...
> [!example] 实验 6-9 ★：真机遥操作 XLeRobot 整理桌面
>
...
> [!example] 实验 6-10 ★：在模拟器中测量同任务的理想控制上限
>
...
> [!example] 实验 6-11 ★★：使用 Gemini Robotics-ER 1.5 驱动 XLeRobot 自主整理桌面
>
...
> [!example] 实验 6-12 ★★：在模拟器中比较三种自主整理桌面的闭环
>
...
> [!example] 实验 6-13 ★★★：同一桌面任务的 RGB 跨环境测试
>
...
![[images/fig7-1.svg]]
*图7-1 Agent 评估体系的四个环节*

## 1 一条评估任务的解剖：τ²-bench 的 telecom 领域

我们先完整解剖 τ²-bench 的 telecom 领域一条真实任务。源码位于仓库的 `../code/chapter7/tau2-bench`，任务文件为 `data/tau2/domains/telecom/tasks_small.json`。
...
> [!example] 实验 7-1 ★：运行 τ²-bench 并对比 τ-bench 的演进
>
...
>
> ![[images/fig7-3.svg]]
> *图7-3 τ²-bench 的双控环境与分层验证*

配套仓库保留了一次运行记录（`../code/chapter7/tau2-bench-eval`），下面分析其中一条成功的运行记录：
...
> [!example] 实验 7-3 ★★：构建基于 Rubric 的用户记忆评估系统
>
> 本实验要求改造第三章的 `../code/chapter3/user-memory-evaluation` 框架，将当前基于简单 LLM-as-a-Judge 的评分机制升级为结构化的多维度 Rubric 评估系统。现有系统使用单一 LLM 调用返回通过/失败加评估理由，缺乏结构化的诊断能力。
>
...
> [!example] 实验 7-4 ★★：Advanced JSON Cards 与 RAG 的对比评估
>
> **目标**：在同一套评估集上公平对比结构化记忆与非结构化检索的优势边界。复用两个第三章项目，在 `../code/chapter3/user-memory-evaluation` 的 60 个测试用例上对比三种配置——纯 Advanced JSON Cards（结构化卡片常驻上下文、无需检索）、纯 RAG（对话分块入向量库、必须检索）、混合系统（核心事实常驻 + 原始对话按需检索）。
...
> [!example] 实验 7-5 ★★：构建全自动 TTS 质量评估流水线
>
...
> [!example] 实验 7-6 ★★：对 AndroidWorld 失败轨迹做失败归因
>
> 本实验用真实轨迹练习本节的归因方法，不需要模拟器，也不需要调用模型 API。素材是 `../code/chapter7/android-world` 中已保存的 T3A 运行记录：`t3a.md` 是全部任务的逐步 `Action`/`Reason`/`Summary`，`t3a_failed.md` 集中收录了 50 余条失败轨迹，每条末尾都带验证器给出的客观判定。
>
...
#### 5.2.3 精确复制错误：从 `old_string` mismatch 到逐层定位
...
> [!example] 实验 7-7 ★★：轨迹前缀边界评估：同一上下文的多种表示
>
...
> [!example] 实验 7-9 ★★：在固定 Coding Harness 中测量模型的行动阈值
>
...
> [!example] 实验 7-10 ★：Agent 任务的端到端成本分析
>
...
> [!example] 实验 7-12 ★★：用户记忆系统的端到端选型评估
>
> **前置要求**：需完成第三章上下文检索或智能体化 RAG 实验。
>
> **目标**：对用户记忆检索 Agent 做全链路选型评估，看嵌入模型、reranker、Agent 主模型三个选择点如何共同影响检索质量、延迟与成本。复用 `../code/chapter3/contextual-retrieval-for-user-memory` 或 `../code/chapter3/agentic-rag-for-user-memory`，在 60 个测试用例上对比。
>
...
> [!example] 实验 7-13 ★★★：AndroidWorld 的评估和改进
>
> 本实验练习如何从评估报告一路走到系统改进。以 `../code/chapter7/android-world` 中的 AndroidWorld 历史报告和三组已保存的配对结果为起点。
>
...
> [!example] 实验 7-14 ★★：配置 OpenVLA 与 RoboTwin2 的具身智能环境
>
...
>
> ![[images/fig7-10.svg]]
> *图7-10 OpenVLA 与 RoboTwin2 具身智能环境*
>
>
...
### 1.2 Mid-training 的本质：在目标分布上继续学习
...
### 2.2 两种动作表示：经典 RL 设置与 LLM 的变长策略
...
> [!example] 实验 8-1 ★：Q-learning 在寻宝游戏中的表现
>
...
> [!example] 实验 8-2 ★★：传统 RL 与 LLM Agent 的对比研究
>
>
> ![[images/fig8-7.svg]]
> *图8-7 Q-learning 与 LLM Agent 在寻宝游戏中的架构对比*
>
>
...
> [!example] 实验 8-3 ★★：从头训练 LLM——算法改进的威力
>
...
> [!example] 实验 8-6 ★★★：语音 SFT——从 “声音复制” 到 “副语言建模” `[扩展实验]`
>
...
> [!example] 实验 8-7 ★★★：多语言思考——让模型用任意语言思考 `[扩展实验]`
>
...
> [!example] 实验 8-8 ★★：Prompt 蒸馏——以更小开销复现可用能力
>
...
> [!example] 实验 8-9 ★★★：思维链（Chain of Thought, CoT）蒸馏
>
...
> [!example] 实验 8-10 ★★：AdaptThink——学会 “何时不思考”
>
...
> [!example] 实验 8-11 ★★：GeneralPoints——单轮 RL 的 “记忆与泛化” 对照
>
>
> ![[images/fig8-12.svg]]
> *图8-12 GeneralPoints 实验架构（GP-L 与 GP-VL 两个变体的训练与测试设计）*
>
>
...
## 9 RL 算法：从 16 次 rollout 到一次参数更新
...
![[images/fig8-13.svg]]
*图8-13 同一 SWE-bench 任务的 16 次 rollout、验证与相对优势*
...
#### 9.1.1 看似 On-Policy，为什么仍会被数值误差拖垮
...
> [!example] 实验 8-12 ★★★：V-IRL-VL——多轮视觉导航
>
...
> [!example] 实验 8-13 ★★★：SimpleVLA-RL——结果奖励下的开放探索 `[扩展实验]`
>
...
> [!example] 实验 8-14 ★★★：ReTool——代码解释器增强数学解题
>
> ![[images/fig8-17.svg]]
> *图8-17 ReTool 交织文本-代码思考与沙盒执行反馈循环*
>
...
> [!example] 实验 8-15 ★★★：AWorld-train——在沙盒中学习使用工具
>
> ![[images/fig8-18.svg]]
> *图8-18 AWorld-train MCP 沙盒训练架构与工具生态*
>
...
> [!example] 实验 8-16 ★★★：RLVP——奖励结果、惩罚路径
>
...
> [!example] 实验 8-17 ★★：从“过早结束”问题案例到 DPO 修复
>
...
> [!example] 实验 8-18 ★★：作用域敏感的中文弯引号 SFT
>
...
> [!example] 实验 8-19 ★★：特殊字符串的精确复制 SFT
>
...
![[images/fig9-2.svg]]
*图9-2 从环境结果到 LLM Rubric 的三层轨迹验证*
...
> [!example] 实验 9-1 ★★：为客服 Agent 构建轨迹验证器
>
...
> [!example] 实验 9-2 ★★：从 τ²-bench 失败轨迹提炼转接与工具使用规则
>
...
> [!example] 实验 9-3 ★★：基于失败轨迹优化航空客服的系统 Prompt
>
...
#### 2.2.2 例子二：需求澄清 Skill——从“直接开工”到“先确认再执行”
...
> [!example] 实验 9-4 ★★：从用户反馈中进化需求澄清与 Spec 确认 Skill
>
...
> [!example] 实验 9-6 ★★★：由失败轨迹触发 Agent 自我修改
>
...
> [!example] 实验 9-7 ★★：由用户反馈触发高风险操作确认门禁
>
...
#### 2.3.1 案例：DeepSeek Harness 以“一切皆插件”实现自我进化
...
> [!example] 实验 9-8 ★★★：把这本书交给 Hermes：它能升级自己吗？
>
...
> [!example] 实验 9-9 ★★★：评估 Agent 是否在持续进化
>
...
> [!example] 实验 10-1 ★★：共享上下文中的多角色转换——系统提示词与 Skill 的对比
>
...
> [!example] 实验 10-3 ★★★：自主编排的电话 + 电脑 Agent
>
...
>
> ![[images/fig10-7.svg]]
> *图10-7 Phone 与 Computer 双 Agent 架构*

> [!example] 实验 10-4 ★★★：同时从多个网站搜集信息的 Agent
>
...
> [!example] 实验 10-6 ★★★：语音狼人杀 Agent 系统
>
